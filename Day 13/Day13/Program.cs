...
namespace Day13
{
    class Program
    {
        private class Day13
        {
            ...
            public Day13(string line1, string line2)
            {
                DepartureTime = long.Parse(line1);
                Buses = line2.Split(',');
            }
        }
        ...
        {
            //file in disk
            var FileUrl = @"D:\Source\Advent of Code\Day 13\day13.txt";
        ...
        private static void Step1(string[] step1)
        {
            Day13 info = new Day13(step1[0], step1[1]);
            List<long> buses = new List<long>();

            foreach (string bus in info.Buses)
            {
                if (bus != "x")
                {
                    buses.Add(long.Parse(bus));
                }
            }
            ...
            foreach (long bus in buses)
            {
                int index = 1;
                long busTime = bus;
                while (busTime <= info.DepartureTime)
                {
                    busTime = bus * index;
                    index++;
                }
                ...
                if (-1 == waitTime)
                {
                    waitTime = busTime - info.DepartureTime;
                    selectedBus = bus;
                }

                if (busTime - info.DepartureTime < waitTime)
                {
                    waitTime = busTime - info.DepartureTime;
                    selectedBus = bus;
                }
            ...
            Console.WriteLine("Step 1 - {0}", selectedBus * waitTime);
        }

        private static void Step2(string[] step2)
        {
            Day13 info = new Day13(step2[0], step2[1]);

            long startTime = 0;
            long increment = 0;
            for (int i = 0; i < info.Buses.Length; i++)
            {
                if (info.Buses[i] != "x")
                {
                    long bus = long.Parse(info.Buses[i]);                    
                    ...
                    {   
                        ...
                        // next increment amount is the period of the alignment
                        increment = bus * increment;
                    }
                }
            }

            Console.WriteLine("Step 2 - {0}", startTime);
        }
    }
}
...
ship, you discover that no ships embark from that port to your vacation island. You'll need to get from the 
...
To save time once you arrive, your goal is to figure out the earliest bus you can take to the airport. (There 
will be exactly one such bus.)
...
wait 944 - 939 = 5 minutes before it departs. Multiplying the bus ID by the number of minutes you'd need to 
wait gives 295.
...
the first bus ID departs at that time and each subsequent listed bus ID departs at that subsequent minute. (The 
