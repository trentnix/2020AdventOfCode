...
namespace Day10
{
    class Program
    {
        ...
        {
            //file in disk
            var FileUrl = @"D:\Source\Advent of Code\Day 10\day10.txt";
        ...
        static void Step1(List<int> adapters)
        {
            int differenceCountOne = 0;
            int differenceCountTwo = 0;
            int differenceCountThree = 0;

            adapters.Add(0);
            adapters.Sort();
            for (int i = 0; i < adapters.Count; i++)
            {
                int joltageDifference = 0;
                if (i == 0)
                {
                    joltageDifference = adapters[i] - 0;
                }
                else
                {
                    joltageDifference = adapters[i] - adapters[i - 1];
                }
            ...
            differenceCountThree++; // for the last one

            Console.WriteLine("Step 1 - {0}(1s) * {1}(3s) = {2}", differenceCountOne, differenceCountThree, differenceCountOne * differenceCountThree);
        }

        static void Step2(List<int> adapters)
        {
            adapters.Remove(0);

            long[] paths = new long[3] { 0, 0, 1 };
            int[] adapterValues = new int[3] { 0, 0, 0 };

            foreach (int adapter in adapters)
            {
                long pathCount = paths[2];
                if (adapter - adapterValues[1] <= 3)
                {
                    pathCount += paths[1];
                    if (adapter - adapterValues[0] <= 3)
                    {
                        pathCount += paths[0];
                    }
                }

                adapterValues[0] = adapterValues[1];
                adapterValues[1] = adapterValues[2];
                adapterValues[2] = adapter;
            ...
            Console.WriteLine("Step 2 - Number of Paths: {0}", paths[2]);
        }
    }
}
...
Your puzzle answer was 24803586664192.
 * 
 */